...
uses
  LCLIntf, LCLType, LMessages, Classes, Graphics, Printers;
    ...
    begin
      ...
      begin
       ...
       // Y_resolution  := GetDeviceCaps( handle, LOGPIXELSY );
        X_resolution := X_resolution div XDPI; //GetDeviceCaps(Handle, LOGPIXELSX);
        Y_resolution := Y_resolution div YDPI; //GetDeviceCaps(Handle, LOGPIXELSY);
       // printorigin.X := GetDeviceCaps( handle, PHYSICALOFFSETX );
       // printorigin.Y := GetDeviceCaps( handle, PHYSICALOFFSETY );
        printorigin.X := PaperSize.PaperRect.WorkRect.Left; //GetDeviceCaps(Handle, PHYSICALOFFSETX);
        printorigin.Y := PaperSize.PaperRect.WorkRect.Top; //GetDeviceCaps(Handle, PHYSICALOFFSETY);
        pagerect.Left := 0;
       // pagerect.Right:= GetDeviceCaps( handle, PHYSICALWIDTH );
        pagerect.Right := PageWidth; // GetDeviceCaps(Handle, PHYSICALWIDTH);
        pagerect.Top  := 0;
        //pagerect.Bottom := GetDeviceCaps( handle, PHYSICALHEIGHT );
        pagerect.Bottom := PageHeight; // GetDeviceCaps(Handle, PHYSICALHEIGHT);
      ...
      begin
        ...
        if (left>=right) or (top>=bottom) then
          raise EPrinter.Create(
            'FAST DosPrint: impossibile settare i margini per la stampante in uso');
      end;
  ...
  begin
    ...
    if (linesPerInch<>0) then
      ...
      if (LinesPerPage<>0) then
        ...
        raise EPrinter.Create(
          'FAST DosPrint - Impossibile stampare a zero righe per pagina.');
  end;
  ...
  procedure PrintPage;
    ...
    begin
      ...
      //    MultiByteToWideChar(852,0,PChar(lines[textStart]),-1,PWideChar(ws),len-1);
          ExtTextOut(Printer.Canvas.Handle,
                      r.Left,r.Top,
                      ETO_CLIPPED,
                      @r,
                      pchar(ws),
...
begin
  assert(Assigned(afont),'FAST DosPrint: richiesto un Font valido!');
  continuePrint := True;
  pagecount     := 1;
  textstart     := 0;
